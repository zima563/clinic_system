...
const createInvoice = async (total, createdBy) => {
...
const createInvoiceDetails = async (id, body, createdBy) => {
    return await prismaClient_1.prisma.invoiceDetail.create({
        data: {
            invoiceId: id,
            createdBy,
...
const listInvoice = async (query) => {
    ...
    const totalSum = await prismaClient_1.prisma.invoice.aggregate({
        _sum: {
            total: true,
        },
...
const getInvoiceDetails = async (id) => {
    return await prismaClient_1.prisma.invoiceDetail.findUnique({
        where: { id },
    });
};
exports.getInvoiceDetails = getInvoiceDetails;
const updateInvoice = async (id, body, invoiceDetail) => {
    if (body.amount) {
        const invoice = await prismaClient_1.prisma.invoice.findUnique({
            where: { id: invoiceDetail.invoiceId },
        });
        if (!(invoice?.total instanceof library_1.Decimal) ||
        ...
        const invoiceTotal = new library_1.Decimal(invoice?.total || 0);
    ...
    await prismaClient_1.prisma.invoiceDetail.update({
        where: { id },
        data: body,
    });
};
exports.updateInvoice = updateInvoice;
const summeryInvoice = async (date, month) => {
    if (!date && !month) {
...
const pdf_summary = async (date, month, res) => {
    if (!date && !month) {
        throw new ApiError_1.default("You must provide either a specific date or a month in the format YYYY-MM.");
    }
    let startDate = new Date();
    ...
    const result = await pdf_creator_node_1.default.create(document, options);
...
const listInvoiceDetails = async (id) => {
...
const appendInvoiceDetail = async (id, invoice, body, req) => {
    const invoiceTotal = new library_1.Decimal(invoice?.total || 0);
    const bodyAmount = new library_1.Decimal(body.amount || 0);
    ...
    await prismaClient_1.prisma.invoiceDetail.create({
        data: {
            invoiceId: id,
...
const removeInvoiceDetail = async (id, invoice, invoiceDetail) => {
    const invoiceTotal = new library_1.Decimal(invoice?.total || 0);
    const invoiceDetailAmount = new library_1.Decimal(invoiceDetail.amount || 0);
    ...
    await prismaClient_1.prisma.invoiceDetail.delete({
        where: { id },
    });
};
exports.removeInvoiceDetail = removeInvoiceDetail;
const getInvoiceById = async (id) => {
    return await prismaClient_1.prisma.invoice.findUnique({
        where: { id },
    });
};
exports.getInvoiceById = getInvoiceById;
const deleteInvoice = async (id) => {
    await prismaClient_1.prisma.invoiceDetail.deleteMany({
        where: {
            invoiceId: id,
        },
    });
