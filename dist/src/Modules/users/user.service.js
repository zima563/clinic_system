...
const addUser = async (res, body) => {
    body.password = bcrypt_1.default.hashSync(body.password, 10);
    let user = await prismaClient_1.prisma.user.create({ data: body });
    return res.status(201).json(user);
};
exports.addUser = addUser;
const getAllUser = async (res, query) => {
    const baseFilter = {
        isDeleted: false,
    };
...
const getUser = async (res, id) => {
    let user = await (0, exports.getUserById)(id);
    if (!user)
...
const changePassword = async (res, id, body) => {
    let user = await (0, exports.getUserById)(id);
    if (!user)
        throw new ApiError_1.default("user not found", 404);
...
const deactiveUser = async (res, id, userId) => {
    let user = await (0, exports.getUserById)(id);
    if (!user)
        throw new ApiError_1.default("user not found", 404);
    if (userId === id) {
...
const getUserPermissions = async (user) => {
    return await prismaClient_1.prisma.userPermission.findMany({
...
const getUserRole = async (user) => {
    return await prismaClient_1.prisma.userRole.findFirst({
        where: { userId: user.id },
        include: { role: true },
    });
};
exports.getUserRole = getUserRole;
const getPermissionRelatedWithRole = async (userRole) => {
    return await prismaClient_1.prisma.rolePermission.findMany({
...
const login = async (res, body) => {
    let user = await (0, exports.findUser)(body);
    if (!(user && bcrypt_1.default.compareSync(body.password, user.password))) {
        ...
        const userPermissions = await (0, exports.getUserPermissions)(user);
        // Fetch user's role
        const userRole = await (0, exports.getUserRole)(user);
        // Fetch permissions related to the user's role
        const rolePermissions = userRole
            ? await (0, exports.getPermissionRelatedWithRole)(userRole)
            : [];
